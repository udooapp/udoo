...
public interface IHistoryRepository extends Repository<History, Integer> {
    ...
    @Query("Select rh From History rh Where rh.hid < :hid and ((rh.type = 0  and rh.tid in (Select u.uid From User u Where u.uid in (Select c.cid From Contact c Where c.uid = :uid)) ) or (rh.type = 2 and rh.tid in (Select r.rid From Request r Where r.completed = true and r.uid in (Select c.cid From Contact c Where c.uid = :uid))) or (rh.type = 1 and rh.tid in (Select o.oid From Offer o Where o.completed = true and o.uid in (Select c.cid From Contact c Where c.uid = :uid)))) order by rh.date DESC ")
    List<History> findAllUidAndHidGreaterThan(@Param("uid") int uid, @Param("hid") int hid, Pageable page);
