...
public class CategoryResultDao extends JdbcDaoSupport implements ICategoryResult {
    ...
    public List<SearchElement> getAllCategories(String text) {
        String sqlO1 = "SELECT distinct c.cid as cid, c.name as name, o.title as text FROM offer o, categories c WHERE o.completed = true and c.cid = o.category AND  LOWER(o.title) LIKE CONCAT('%',LOWER('" + text + "'),'%') AND o.expirydate > CURRENT_DATE AND o.uid NOT IN (SELECT v.uid FROM verification v Where v.type = 0) Group By c.cid, c.name, o.title";
        ...
        String sqlR1 = "SELECT distinct c.cid as cid, c.name as name, r.title as text FROM request r, categories c WHERE r.completed = true and c.cid = r.category AND  LOWER(r.title) LIKE CONCAT('%',LOWER('" + text + "'),'%') AND r.expirydate > CURRENT_DATE AND r.uid NOT IN (SELECT v.uid FROM verification v Where v.type = 0) Group By c.cid, c.name, r.title";
    ...
    public String getWordMatch(String text) {
        if (!text.isEmpty()) {
            String sql1 = "SELECT r.title as title FROM request r WHERE r.completed = true and LOWER(r.title) LIKE CONCAT('%', LOWER('" + text + "'), '%')";
            String sql2 = "SELECT o.title as title FROM offer o WHERE o.completed = true and LOWER(o.title) LIKE CONCAT('%', LOWER('" + text + "'), '%')";
