...
export class InputPage {
  ...
  public getParagraphText() {
    browser.wait(protractor.ExpectedConditions.presenceOf(element(by.css('app-root h1'))), 1000);
    return element(by.css('app-root h1')).getText();

  }
  public getElementText(id: string) {
    browser.wait(protractor.ExpectedConditions.presenceOf(element(by.id(id))), 1000);
    return element(by.id(id)).getText();

  }

  public setChecked(id: string) {
    browser.wait(protractor.ExpectedConditions.presenceOf(element(by.css("label[for='" + id + "']"))), 1000);
    element(by.css("label[for='" + id + "']")).click();
  }

  public clearText(id: string){
    browser.wait(protractor.ExpectedConditions.presenceOf(element(by.id(id))), 1000, 'Element taking too long to appear in the DOM');
